...
## 2026-04-22 — Session 092: Повне розділення prod і multitenant (git, сервер, БД)

### Контекст

Після двох попередніх rollback-ів prod був рятований наспіх, а git/сервер залишилися в хаотичному стані: `origin/master` вів на легасі 2024 (`10d0241`), було ~25 codex-бранчів, у prod DB і prod `.env` стирчали multitenant-таблиці й ключі, staging-бот не мав systemd-юніта, а `deploy.cjs` міг знову випадково перезаписати prod. Задача: розвести два проекти остаточно так, щоб майбутня помилка була неможлива.

### Що тепер де

| Шар | Prod (stable, pre-multitenant) | Staging (multitenant WIP) |
|---|---|---|
| Гілка git | `master` → `cd36804` | `multitenant` → `21bd10a` |
| Каталог на сервері | `/opt/smartest/app` | `/opt/smartest-staging` |
| venv | `/opt/smartest/venv` | `/opt/smartest-staging/.venv` |
| `.env` | `/opt/smartest/.env` | `/opt/smartest-staging/.env` |
| Telegram bot | `@saintaibot` (token `6553...`) | `@chibigochibot` (token `8377...`) |
| systemd юніти | `smartest-bot` + `smartest-admin` (порт 8787) | `smartest-staging-bot` + `smartest-staging-admin` (порт 8788) |
| Домен через Caddy | `smartest.klawa.top` → `127.0.0.1:8787` | `test.klawa.top` → `127.0.0.1:8788` |
| БД | `smartest_db` (user `smartest`) — тільки pre-multitenant таблиці | `aisus_test` (user `aisus`) — multitenant schema |
| Billing / users / topups | немає (prod про це не знає) | працюють, адмін = `311422683` |

### Git: що зроблено

- `origin/master` форспушений з легасі 2024 (`10d0241`) на `cd36804` — це справжній stable prod baseline.
- Поточний multitenant WIP закомічений як `21bd10a Multitenant WIP: audit findings + portal scaffolding` і штатно запушений в `origin/multitenant`.
- З `origin` видалено 23 мертві гілки: усі `codex/*` (12 intergrate-database, 3 add-telegram-bot, 3 separate-api-keys, 4 separate-services, 1 emotional voices), плюс `dall-e`, `refactor_t1/2`, `single_file`, `text_file_support`, `web_search`, `smartest-docs-no-devlog`, `refactoring_subfunctions`, `gpt_wiser`, `backup/pre-rewrite`.
- Локально залишились тільки `master` і `multitenant`; решта локальних бранчів (claude worktree, dall-e, golovna, gpt_wiser, main, refactoring_subfunctions, smartest-docs-no-devlog) видалені.
- `.gitignore` доповнений: `.tmp/`, `.claude/`, `.zed/`, `.secrets/`, `mariadb-*.zip`, `deploy/*.tar.gz`.
- `heroku` remote лишено як є (окремий deploy target, git-простору origin не захаращує).

### Сервер: prod (`/opt/smartest`)

- Видалені backup-каталоги: `app.prev`, `app.experimental_20260422181936`, `app.multitenant_20260422182417` (≈37MB), `.env.backup-20260403-124358`.
- З prod `.env` видалено `BILLING_MASTER_KEY` (pre-multitenant код його не читає).
- Pre-multitenant prod файли лишилися без змін: `run.py`, `app/admin_ui.py`, `db/connection.py` співпадають з `cd36804` (md5 після `\r`-стрипу однаковий).
- `smartest-bot.service` і `smartest-admin.service` продовжують працювати без рестарту.

### Сервер: staging (`/opt/smartest-staging`)

- Створено `smartest-staging-bot.service` з `WorkingDirectory=/opt/smartest-staging`, `ExecStart=/opt/smartest-staging/.venv/bin/python run.py`, логи в `logs/bot.log` і `logs/bot.err.log`. `systemctl enable` + `daemon-reload`.
- Оновлено `smartest-staging-admin.service` версію прямо у репо (`deploy/smartest-staging-admin.service`), щоб була еталонною. На сервері він уже крутився.
- `deploy/deploy.cjs` переписаний під явний `--target=prod|staging` (без default), різні шляхи, venv і services. Без `--target` скрипт валиться з помилкою — накривний guard проти повторення Codex-інциденту.
- Код `multitenant@21bd10a` задеплоєний у `/opt/smartest-staging` через `node deploy/deploy.cjs --target=staging`. Обидва сервіси `active`, admin відповідає на `https://test.klawa.top/` (Caddy → 127.0.0.1:8788).
- Poll-бот стартує штатно, але логи показують `telegram.error.Conflict: terminated by other getUpdates request` — це зовнішній процес десь ще тримає long-poll на той самий тестовий токен (не на сервері, на сервері процес один). Потрібна перевірка dev-машин; сервер не є джерелом конфлікту.

### БД

**Prod (`smartest_db`)** очищена від multitenant-таблиць. Дропнуті: `accounts`, `chat_access`, `chat_policies`, `pricing`, `provider_keys`, `topups`, `transactions`, `users`, `user_settings`, `turns`. Лишилися рівно ті, що створює cd36804 (`001_init.sql`, `002_search_cache.sql`, `003_memory_core.sql`): `chats`, `participants`, `glossary`, `threads`, `messages`, `memory_recent`, `memory_long`, `memory_core`, `settings`, `search_cache`, `page_cache`, `migrations_log`.

**Staging (`aisus_test`)** зачищена від тестових профілів: видалено `user_id = 7 (oleh)` і `999000111 (staging_probe)` разом з їхніми `accounts`. Лишився єдиний адмін `user_id = 311422683 (AgNike)` зі своїм `account_id = 2`. Reference-таблиця `pricing` (27 рядків з `gpt-5.4*`, `gpt-4.1*` та ін.) залишена.

### Deploy script guard

Новий контракт `deploy.cjs`:

```
node deploy/deploy.cjs --target=prod     # -> /opt/smartest/app, @saintaibot, smartest.klawa.top
node deploy/deploy.cjs --target=staging  # -> /opt/smartest-staging, @chibigochibot, test.klawa.top
```

Без `--target` → error з описом обох варіантів і `exit 1`. Випадковий запуск без прапорця більше не перезапише prod.

### Що залишилось на Session 093+

- Прибрати Telegram Conflict на тестовому токені: знайти джерело сторонніх getUpdates (імовірно dev-машина), зупинити.
- Реалізувати пріоритетні пункти з `multitenant-plan.md` Section 12: event-loop blocking (Variant 1: `asyncio.to_thread` навколо sync `chat_once` у `runner.py`, `planner.py`, `search_task.py`, `gateway.py`), seed `ADMIN_TG_USER_IDS`, тести `test_094_concurrent_llm.py`.
- Дооформити staging smoke: поповнення балансу, transaction logging, keypool rotation під `@chibigochibot`.
- Heroku remote (app `tranquil-thicket-16349`) — вирішити, чи видалити (окремий деплой, не заважає).
