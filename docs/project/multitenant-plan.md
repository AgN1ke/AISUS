...
**Статус:** частково реалізовано (Stages 1-4 done, Stage 4.5A done; §12.3 Частини А-Б виконані, pricing seed + Gemini usage fix закриті, keypool-first policy для billed turns виконана, `/balance` breakdown закритий, voice/persona в Telegram `/settings` закриті; активний спринт — portal/login і подальша runtime зачистка)
...
|------|--------|---------------|
| 1 — DB schema | ✅ | — |
| 2 — Key pool + Gateway | ⚠️ | runtime вже бере ключі з keypool пріоритетно для billed turns, `pricing` автосіється під час bootstrap, Gemini usage extraction виправлений; незакритий хвіст тут уже не policy, а операційне засівання `provider_keys`, щоб env-fallback майже не використовувався |
| 3 — Policy + debit | ✅ | — |
...
**P9. Не всі 131 зміни закоммічені, бранч `gpt_wiser` змішує stage 1-4.5A і ранні memory-сесії.**

`git status` показує 131 модифікований файл. `git log` останнім стоїть `cd36804 Fix .env encoding` — тобто весь Stage 1, 2, 3, 4, 4.5A, model_preferences, admin dashboard, нові тести 087-093 — усе висить uncommitted. Це блокує і деплой (не знаємо, що саме котимо), і rollback (немає `multitenant`-гілки з чистою базою для `git revert`).

**Як фіксити:** створити гілку `multitenant` від master, черешнями перекинути зміни по стадіях (5-6 комітів: 1-schema, 2-keypool+gateway, 3-policy, 4-telegram-ui, 4.5A-admin, misc-fixes). Тоді деплой зводиться до `git checkout multitenant && pm2 restart`, rollback — `git checkout master && pm2 restart`.
...
8. **P1** — після переходу на `chat_once_async` (Варіант 2 з Частини А) — зняти `_run_async_sync`.
9. **P9** — розібратись із 131 uncommitted змінами, винести їх у гілку `multitenant` серією комітів за стадіями.
10. **P5** — мігрувати model catalog з хардкоду `model_preferences.py` на БД (`provider_catalog` або `pricing`). Можна робити після першого публічного релізу.
