...
async def insert_message(
    ...
    has_media: bool,
    thread_root_msg_id: Optional[int],
):
...
async def get_message(chat_id: int, msg_id: int) -> Optional[dict]:
    return await fetchone(
        "SELECT * FROM messages WHERE chat_id=%s AND msg_id=%s", (chat_id, msg_id)
    )


async def fetch_thread_messages(
    chat_id: int, root_id: int, limit: int = 20
) -> List[dict]:
...
async def get_term(chat_id: int, term: str) -> Optional[dict]:
    return await fetchone(
        "SELECT * FROM glossary WHERE chat_id=%s AND term=%s", (chat_id, term)
    )


async def fetch_gc_candidates(
    chat_id: int, idle_days: int = 30, limit: int = 10
) -> List[dict]:
    return await fetchall(
        """
