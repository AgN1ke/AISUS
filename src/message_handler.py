...
class CustomMessageHandler:
    def __init__(self, config: ConfigReader, voice_processor: VoiceProcessor, chat_history_manager: ChatHistoryManager):
        ...
        self.openai_client = None  # Ініціалізуємо клієнт пізніше
    ...
    async def _handle_user_message(self, bot, message: MessageWrapper):
        user_input, is_voice = await self._process_message_content(message)
        if user_input is None:
            print("Повідомлення користувача не знайдено.")
            return

        chat_id = message.chat_id
        print(f"Обробка повідомлення від {message.from_user_first_name} ({chat_id})")

        # Ініціалізуємо клієнт Realtime API
        self.openai_client = OpenAIRealtimeClient(api_key=self.config.get_openai_settings()['api_key'])
        await self.openai_client.connect()

        if is_voice:
            await self.openai_client.send_audio(user_input)
        else:
            await self.openai_client.send_message(user_input)

        # Отримуємо відповіді та відправляємо їх користувачу
        response_text, response_audio = await self.openai_client.receive_responses()

        if response_audio:
            # Зберігаємо аудіо та відправляємо його користувачу
            output_audio_path = f"response_{chat_id}.wav"
            self.voice_processor.save_audio_from_base64(base64.b64encode(response_audio).decode('utf-8'), output_audio_path)
            await message.reply_voice(voice=open(output_audio_path, 'rb'))
            ...
            await message.reply_text("Не вдалося отримати відповідь від бота.")
