...
from core.user_preferences import voice_option
...
def resolved_voice_id() -> str:
    configured = (current_runtime_user_settings().get("voice_id") or "").strip().lower()
    if configured and voice_option(configured):
        return configured
    return vocalizer_voice()
...
def normalize_tts_text(text: str) -> str:
    ...
    value = _MARKDOWN_LINK_RE.sub(lambda match: match.group(1).strip(), value)
...
async def _send_telethon_voice(
    msg: UnifiedMessage, path: str, reply_to: int | None
) -> None:
    kwargs = {"voice_note": True}
    if reply_to is not None:
        kwargs["reply_to"] = reply_to
    await msg.raw_update.reply(file=path, **kwargs)
    ...
    try:
        for index, path in enumerate(paths):
            ...
            if msg.platform == "ptb":
                ...
                await _send_telethon_voice(msg, path, chunk_reply_to)
    finally:
