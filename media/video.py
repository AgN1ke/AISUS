...
import logging, os, subprocess, shlex, math, shutil
...
def _ffprobe_duration_sec(path: str) -> float:
    cmd = f"{FFPROBE_BIN} -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 {shlex.quote(path)}"
    try:
        out = subprocess.check_output(cmd, shell=True, text=True).strip()
        return float(out)
    except Exception as e:
        logger.error("ffprobe failed for %s: %s", path, e, exc_info=True)
        return 0.0

def _extract_audio_mp3(video_path: str, out_mp3: str, kbps: int = 96) -> None:
    cmd = f"{FFMPEG_BIN} -y -i {shlex.quote(video_path)} -vn -acodec libmp3lame -b:a {kbps}k {shlex.quote(out_mp3)}"
...
def _sample_frames(video_path: str, out_dir: Path, every_sec: float, max_frames: int) -> List[str]:
    ...
    cmd = f"{FFMPEG_BIN} -y -i {shlex.quote(video_path)} -vf 'fps={fps}' {shlex.quote(str(tmp_dir / 'frame_%05d.jpg'))}"
...
def analyze_video(video_path: str, task_hint: str | None = None) -> dict:
    ...
    try:
        ...
        txt_src = Path(mp3).with_suffix(".txt")
        if CLEANUP_KEEP_WHISPER_TXT and txt_src.exists():
            shutil.move(str(txt_src), str(base.with_suffix(".txt")))
        elif txt_src.exists():
            txt_src.unlink(missing_ok=True)
    except Exception as e:
        logger.error("audio extraction/transcription failed for %s: %s", video_path, e, exc_info=True)
        raise
    finally:
        try:
            os.remove(mp3)
    ...
    try:
        ...
        logger.error("frame sampling/vision failed for %s: %s", video_path, e, exc_info=True)
