...
def _load_env():
    ...
    os.environ.setdefault("OPENAI_REASONING_MODEL", "")
...
async def _db_migrated(_load_env):
    # Apply migrations once, then close the pool so each test can reopen it
    ...
    def create(self, **kw):
        ...
        if kw.get("tools"):
            ...
            if "ПОШУК" in text:
                ...
                fn = Fn()
                fn.name = "search_web"
                fn.arguments = '{"query":"test query","max_results":2}'
                tc.function = fn
                msg.tool_calls = [tc]
                msg.content = None
            else:
