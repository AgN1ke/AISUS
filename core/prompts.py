...
PLANNER_SYSTEM_PROMPT = _block(
    ...
    - search — користувач хоче, щоб бот отримав ЗОВНІШНІ ДАНІ або ДОКАЗИ:
      актуальні дані з інтернету, перевірку по відкритих джерелах, посилання,
      джерела, цитати, OSINT/open-source lookup, підтвердження або спростування
      твердження через web retrieval. Це search навіть якщо сама тема історична
      або стабільна, бо користувач просить не "згадай із пам'яті", а "перевір
      зовні";
    ...
    Search картина: користувач явно або очевидно просить зовнішній retrieval:
    знайти/перевірити/пробити/зібрати evidence у вебі або відкритих джерелах.
    Не прив'язуйся до конкретних слів: "OSINT", "відкриті джерела",
    "перевір джерела", "обшукай інтернет", "дай посилання", "знайди
    підтвердження" — це одна семантика search-intent. Якщо запит лише про
    принципи роботи, теорію, історію, lore ігор/книг, етимологію чи фольклор
    і користувач НЕ просить зовнішньої перевірки/джерел — це CHAT. Якщо
    сумніваєшся між search і chat — обирай chat. Окремий вузький класифікатор
    далі ще раз перевірить твій search-вибір; задача classifier-а — відсікти
    зайві search-и.
...
SEARCH_GATE_SYSTEM_PROMPT = _block(
    ...
    Твоя задача: визначити, чи треба боту йти в інтернет / web retrieval /
    відкриті джерела саме для відповіді на last_user_message.

    Відповідай ТІЛЬКИ одним словом: SEARCH або CHAT.

    ДВА КРИТЕРІЇ SEARCH:

    1. EXTERNAL-EVIDENCE INTENT:
    Користувач прямо або семантично просить не відповідь із пам'яті, а зовнішню
    перевірку: знайти в інтернеті, перевірити джерела, дати посилання, знайти
    цитату/документ/paper, пробити по OSINT/open sources, обшукати web,
    підтвердити або спростувати твердження через відкриті джерела. Це SEARCH
    навіть для історичних або стабільних тем.
    ...
    SEARCH — коли треба ЗОВНІШНІ ДАНІ:
    - явне або семантичне прохання погуглити, пошукати, перевірити в інтернеті,
      перевірити джерела, знайти посилання, джерело, документ, цитату або
      OSINT/open-source evidence;
    ...
    - будь-яка теорія, принципи, механіка, пояснення «як / чому щось працює»,
      якщо користувач не просить зовнішньої перевірки чи джерел;
    ...
    - дискусія, гіпотеза, мисленнєвий експеримент, контекстне уточнення, якщо
      last_user_message не просить зовнішній retrieval.
    ...
    - «Що робити коли X», «як вчинити при Y» — CHAT, якщо X/Y стабільна
      ситуація і користувач не просить зовнішньої перевірки. Модель знає
      принципи й кроки дії, актуального вебу не треба.
...
SEARCH_COMPOSER_SYSTEM_PROMPT = _block(
    ...
    - Прибирай дієслова-команди і службову оболонку external lookup: прохання
      пошукати/погуглити/перевірити, "пробий по OSINT", "обшукай інтернет",
      "перевір джерела", "дай посилання" тощо. У query лишай саму тему, яку
      треба знайти або перевірити.
    ...
    - Якщо вона двозначна типу "ну загугли", "пробий це", "перевір ще раз",
      витягни тему з найближчого релевантного повідомлення у dialogue_excerpt,
      а не з випадкової старої теми.
    """
)
...
SEARCH_QUERY_PLANNER_PROMPT = _block(
    ...
    - ніколи не змінюй тему запиту. Sub-query має зберігати основні сутності
      з `latest_user_message` або з уже розв'язаного контекстного запиту;
      якщо контекст неясний, краще поверни один консервативний sub-query, ніж
      вигадуй іншу тему;
